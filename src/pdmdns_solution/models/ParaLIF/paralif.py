...
class ParaLIF(Base):
    ...
    def compute_params_fft(self, nb_steps):
        ...
        # if no time constant, no need to perform leaky integration
        if (self.alpha==0).all() and (self.beta==0).all() and not self.learn_tau: return None
    ...
    def parallel_leaky_integrate(self, X):
        if self.fft_l_k is None: return X
        fft_X = torch.fft.rfft(X, n=2*self.nb_steps, dim=1)
        X_integrated = torch.fft.irfft(fft_X * self.fft_l_k, n=2*self.nb_steps, dim=1)[:, :self.nb_steps, :]
        return X_integrated
    ...
    def forward(self, X, parallel=True):
        ...
        if self.nb_steps != nb_steps or self.learn_tau or X.device != self.device: 
            self.fft_l_k = self.compute_params_fft(nb_steps)
            self.nb_steps = nb_steps

        # Perform parallel leaky integration - Equation (15)
        mem_pot_hidden = self.parallel_leaky_integrate(X)
        ...
        if self.recurrent:
            ...
            mem_pot_temp = self.parallel_leaky_integrate(X + self.fc_recu(hidden_state))
...
class SpikingFunction(torch.nn.Module):
    ...
    def __init__(self, spike_mode, surrogate_mode, surrogate_scale, device):
        ...
        self.spike_mode_list = ["GS", "SB", "TRB", "D", "SD", "TD", "TRD", "T", "ST", "TT", "TRT"]+["D2", "T2"]
        ...
        elif spike_mode in ["D2"]:
            self.surrogate_fn = SurrogateFunction(mode=surrogate_mode, scale=surrogate_scale)
            self.generate = self.delta_fn2  # Use Delta spike generation
        elif spike_mode in ["T", "ST", "TT", "TRT"]:
            self.surrogate_fn = SurrogateFunction(mode=surrogate_mode, scale=surrogate_scale)
            self.generate = self.threshold_fn  # Use Threshold spike generation
        elif spike_mode in ["T2"]:
            self.surrogate_fn = SurrogateFunction(mode=surrogate_mode, scale=surrogate_scale)
            self.generate = self.threshold_fn2  # Use Threshold spike generation
        elif spike_mode == "GS":
    ...
    # Delta (Onset: +1 - Offset: -1) Spikes Generation - Uses surrogate gradient
    def delta_fn2(self, inputs, spk_threshold):
        """
        Generate spikes using the Delta method with a surrogate gradient.
        ...
        """
        inputs_previous = F.pad(inputs, (0, 0, 1, 0), "constant", 0)[:, :-1]  # Previous time step inputs
        return self.surrogate_fn((inputs - inputs_previous) - spk_threshold)-self.surrogate_fn(-(inputs - inputs_previous) - spk_threshold)  # Spike based on change from previous step
    ...
    # Threshold Spikes Generation - (Onset: +1 - Offset: -1) Simple threshold comparison
    def threshold_fn2(self, inputs, spk_threshold):
        """
        Generate spikes using a simple threshold comparison.
        ...
        - tensor: The generated spike tensor.
        """
        return self.surrogate_fn(inputs - spk_threshold)-self.surrogate_fn(-inputs - spk_threshold)
    ...
    def extra_repr(self):
        ...
        if self.spike_mode[-1]=="2": spike_mode_text += desc[self.spike_mode[-2]] + "2"
        else: spike_mode_text += desc[self.spike_mode[-1]]
