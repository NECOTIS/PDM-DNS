...
class Base(torch.nn.Module):
    ...
    def __init__(self, n_neuron, recurrent, fire, recurrent_fire, spk_threshold, learn_threshold, tau_mem, tau_syn, time_step, learn_tau, device, debug):
        ...
        self.learn_tau_syn = learn_tau and tau_syn is not None #if tau_syn is None its not learned
        self.learn_tau_mem = learn_tau and tau_mem is not None # if tau_mem is None its not learned
        self.learn_tau = self.learn_tau_syn or self.learn_tau_mem
        self.device = device
        self.debug = debug
        ...
        beta = torch.exp(-time_step / torch.tensor(tau_mem, device=self.device)) if tau_mem is not None else torch.tensor(0., device=self.device)
        if self.learn_tau_syn:  
            self._alpha = torch.nn.Parameter(-torch.log((1 / (alpha + 1e-8)) - 1)) #torch.nn.Parameter(alpha)
        else:
            self.register_buffer('_alpha', alpha)
        if self.learn_tau_mem:  
            self._beta = torch.nn.Parameter(-torch.log((1 / (beta + 1e-8)) - 1)) #torch.nn.Parameter(beta)
        else:
        ...
        if self.recurrent:
            self.fc_recu = torch.nn.Linear(self.n_neuron, self.n_neuron, device=self.device, bias=False)
    ...
    @property
    def alpha(self):
        return self._alpha.sigmoid() if self.learn_tau_syn else self._alpha
    ...
    @property
    def beta(self):
        return self._beta.sigmoid() if self.learn_tau_mem else self._beta
