...
class LIF(Base):
    ...
    def forward_step(self, X, syn_cur, mem_pot, refractory_time=None):
        """
        Perform the forward pass of the LIF neuron model for one timestep.

        Parameters:
        - X (tensor): Input tensor with shape (batch_size, input_size).
        - syn_cur (tensor): Synaptic current tensor with shape (batch_size, input_size).
        - mem_pot (tensor): Membrane potential tensor with shape (batch_size, input_size).
        ...
        - If 'debug' is True: Returns a tuple (spiking tensor, membrane potential tensor) for additional insight.
        """
        spikes_t = torch.zeros_like(X)  # shape: [batch_size, n_neuron]

        if self.refractory and refractory_time is not None: refractory_time -= 1
        # Update synaptic current with input contribution - Integrating input to synaptic current (Equation 5)
        syn_cur = self.alpha * syn_cur + X
        
        # Add recurrent input to synaptic current if recurrent connections are enabled (Equation 20)
        if self.recurrent:
            # Choose the source of recurrent input based on `recurrent_fire` flag
            prev_output = X if self.recurrent_fire else F.relu(mem_pot)
            syn_cur += self.fc_recu(prev_output)
        
        # Update membrane potential with synaptic current contribution - Integrating synaptic current (Equation 6)
        mem_pot = self.beta * mem_pot + (1 - self.beta) * syn_cur * (torch.where(refractory_time > 0, 0.0, 1.0) if (self.refractory and refractory_time is not None) else 1.)
        
        if self.fire:
            # Generate spikes based on the membrane potential exceeding the threshold (Equation 3)
            spikes_t = self.spike_fn(mem_pot - self.spk_threshold)
            if (self.refractory and refractory_time is not None): refractory_time[spikes_t==1] = self.refractory
            # Reset membrane potential where spikes occurred (Equation 6)
            mem_pot = mem_pot * (1 - spikes_t.detach())
        
        if self.fire:
            # Calculate the average number of spikes per neuron for potential analysis
            self.nb_spike_per_neuron = torch.mean(spikes_t, dim=0)
        return spikes_t, syn_cur, mem_pot
