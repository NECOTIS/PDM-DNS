    ...
    while file_num <= params["fileindex_end"]:
        try:
            ...
            clean, clean_sf, clean_cf, clean_laf, clean_index = gen_audio(
                is_clean=True, params=params, index=clean_index
            )
            # generate noise
            noise, noise_sf, noise_cf, noise_laf, noise_index = gen_audio(
                is_clean=False, params=params, index=noise_index, audio_samples_length=len(clean)
            )
        ...
        for i in range(len(audio_signals)):
            try:
                ...
                print(str(e))
        # Added
        print(file_num,"/",params["fileindex_end"])
...
def synthesize(params):
    ...
    # Lock to synchronize access to the global variables
    lock = threading.Lock()

    def process_file(file_num):
        nonlocal clean_source_files, clean_clipped_files, clean_low_activity_files
        nonlocal noise_source_files, noise_clipped_files, noise_low_activity_files
        nonlocal clean_index, noise_index
    
        # Perform the processing task
        # Acquire the lock to safely update the global indexes
        with lock:
            current_clean_index = clean_index
            current_noise_index = noise_index
            # Update the global indexes for the next thread
            clean_index += 1
            noise_index += 1

        try:
            # Generate clean speech
            clean, clean_sf, clean_cf, clean_laf, current_clean_index_ = gen_audio(
                is_clean=True, params=params, index=current_clean_index
            )
            # Generate noise
            noise, noise_sf, noise_cf, noise_laf, current_noise_index_ = gen_audio(
                is_clean=False, params=params, index=current_noise_index, audio_samples_length=len(clean)
            )
            clean_clipped_files += clean_cf
            clean_low_activity_files += clean_laf
            noise_clipped_files += noise_cf
            ...
            print(str(e))
            print("Trying again")
            return None  # Return incremented indices

        # Check for unexpected clipping
        if is_clipped(clean_snr) or is_clipped(noise_snr) or is_clipped(noisy_snr):
            print(f"Warning: File #{file_num} has unexpected clipping, skipping...")
            return None
        ...
        # Write the audio signals to disk
        for i in range(len(audio_signals)):
            try:
                audiowrite(file_paths[i], audio_signals[i], params["fs"])
            except Exception as e:
        ...
        with lock:
            # Update the global indexes for the next thread
            clean_index += current_clean_index_ - current_clean_index
            noise_index += current_noise_index_ - current_noise_index
        return file_num

    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = []
        while file_num <= params["fileindex_end"]:
            futures.append(executor.submit(process_file, file_num))
            file_num += 1

        for future in concurrent.futures.as_completed(futures):
            new_file_num = future.result()
            if new_file_num:
                print(new_file_num, "/", params["fileindex_end"])
...
if __name__ == "__main__":
    ...
    seed = 5 if not params["is_validation_set"] else 0
    np.random.seed(seed)
    random.seed(seed)
    ...
    params["fileindex_end"] = params["num_files"] - 1
    ...
    print("num_cleanfiles", params["num_cleanfiles"])

    params["noisefilenames"] = noise_filenames
    params["num_noisefiles"] = len(noise_filenames)
    print("num_noisefiles", params["num_noisefiles"])
